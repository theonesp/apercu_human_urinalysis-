---
title: "Apercu: 02_train_set150Aing_model"
...
There are more methods that can be addressed [Available Models in caret::train_set150A](https://rdrr.io/cran/caret/man/models.html)

```{r eval=FALSE, include=FALSE}
# defining outcome, exposures

outcome_model <- 'bact'
exposures_model <- names(train_set150A)[grepl("spectrum", names(train_set150A))]
...
# gbmFit_set150A <- train( outcome_and_exposure
#                 ,data = train_set150A,
#                 method = "gbm",
...
#                 task_type = "GPU"
# )
svmFit_set150A <- train( outcome_and_exposure
                ,data = train_set150A,
                method = "svmRadial",
                trControl = fitControl,
...
rfFit_set150A <- train( outcome_and_exposure
                ,data = train_set150A,
                method = "rf",
                trControl = fitControl,
                verbose = T,
...
xgbFit_set150A <- train( outcome_and_exposure
                ,data = train_set150A,
                method = "xgbTree",
                trControl = fitControl,
                verbose = T,
...
lrFit_set150A <- train( outcome_and_exposure
                ,data = train_set150A,
                method = "LogitBoost",
                trControl = fitControl,
                verbose = T,
...
```{r}
resamps <- resamples(list( #gbmFit = gbmFit_set150A
                          svmFit = svmFit_set150A
                          ,rfFit  = rfFit_set150A
                          ,xgbFit = xgbFit_set150A
                          #,nnFit = nnFit   # nn failed so we are not test_set150Aing it
                          ,lrFit = lrFit_set150A
                          #,gamFit = gamFit
                          ))
...
#manually select it
best_performing_model<-rfFit_set150A
...
# Evaluating the predictor on our test_set150A dataset

## Creating prediction-probabilities dataset

```{r}
prediction_probabilities<-predict(rfFit_set150A, newdata = test_set150A,type = "prob") # We create the probabilities dataset using our best performing model.
final_predictions<-cbind(test_Y_set150A,prediction_probabilities) # we bind our prediction with the actual data
final_predictions<-rename(final_predictions, obs = test_Y_set150A) # the function twoClassSummary reads the actual outcome as 'obs'
...
```{r}
prediction_probabilities_svm<-predict(svmFit_set150A, newdata = test_set150A, type = "prob") # We create the probabilities dataset using our best performing model.
final_predictions_svm<-cbind(test_Y_set150A, prediction_probabilities_svm) # we bind our prediction with the actual data
final_predictions_svm<-rename(final_predictions_svm, obs = test_Y_set150A) # the function twoClassSummary reads the actual outcome as 'obs'
# NEEDS TO BE CHANGED FOR EVERY MODEL DEPENDING ON THE OUTCOME!!!
...
```{r}
prediction_probabilities_gbm<-predict(gbmFit_set150A, newdata = test_set150A, type = "prob") # We create the probabilities dataset using our best performing model.
final_predictions_gbm<-cbind(test_Y_set150A, prediction_probabilities_gbm) # we bind our prediction with the actual data
final_predictions_gbm<-rename(final_predictions_gbm, obs = test_Y_set150A) # the function twoClassSummary reads the actual outcome as 'obs'
# NEEDS TO BE CHANGED FOR EVERY MODEL DEPENDING ON THE OUTCOME!!!
...
```{r}
prediction_probabilities_rf<-predict(rfFit_set150A, newdata = test_set150A, type = "prob") # We create the probabilities dataset using our best performing model.
final_predictions_rf<-cbind(test_Y_set150A, prediction_probabilities_rf) # we bind our prediction with the actual data
final_predictions_rf<-rename(final_predictions_rf, obs = test_Y_set150A) # the function twoClassSummary reads the actual outcome as 'obs'
# NEEDS TO BE CHANGED FOR EVERY MODEL DEPENDING ON THE OUTCOME!!!
...
```{r}
prediction_probabilities_xgb<-predict(xgbFit_set150A, newdata = test_set150A, type = "prob") # We create the probabilities dataset using our best performing model.
final_predictions_xgb<-cbind(test_Y_set150A, prediction_probabilities_xgb) # we bind our prediction with the actual data
final_predictions_xgb<-rename(final_predictions_xgb, obs = test_Y_set150A) # the function twoClassSummary reads the actual outcome as 'obs'
# NEEDS TO BE CHANGED FOR EVERY MODEL DEPENDING ON THE OUTCOME!!!
...
```{r}
prediction_probabilities_lr<-predict(lrFit_set150A, newdata = test_set150A, type = "prob") # We create the probabilities dataset using our best performing model.
final_predictions_lr<-cbind(test_Y_set150A, prediction_probabilities_lr) # we bind our prediction with the actual data
final_predictions_lr<-rename(final_predictions_lr, obs = test_Y_set150A) # the function twoClassSummary reads the actual outcome as 'obs'
# NEEDS TO BE CHANGED FOR EVERY MODEL DEPENDING ON THE OUTCOME!!!
...
```{r}
#AUC per method
auc_svm<-auc(roc(relabel(test_Y_set150A),relabel(final_predictions_svm$pred) ))
auc_rf<-auc(roc(relabel(test_Y_set150A),relabel(final_predictions_rf$pred) ))
auc_xgb<-auc(roc(relabel(test_Y_set150A),relabel(final_predictions_xgb$pred) ))
auc_lr<-auc(roc(relabel(test_Y_set150A),relabel(final_predictions_lr$pred) ))
...
ggplot(varImp(rfFit_set150A, scale = T))+theme_minimal() 
dataset_1_varImp<-varImp(rfFit_set150A, scale = T)
